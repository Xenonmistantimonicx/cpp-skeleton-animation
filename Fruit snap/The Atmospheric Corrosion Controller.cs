...
public class AAA_NeodymiumDecayController : MonoBehaviour
{
    [System.Serializable]
    public struct IntergranularLatticeNode
    {
        ...
        public float oxideVolumeSwell;        // Accumulation volume of the grey-purple crust
    ...
    [SerializeField] private float atmosphericHumidity = 1.35f; // Ambient moisture velocity multiplier

    [Header("Mechanical Mineral Rigidity")]
    [Tooltip("The initial structural load strength of pure neodymium metal. It is soft and easily deformed compared to steel.")]
    [SerializeField] private float pristineMetalYieldMPa = 145.0f; 
    ...
    [SerializeField] private ParticleSystem greyPurpleOxideDustFX; // Brittle, chalky purple-grey debris flakes

    private List<IntergranularLatticeNode> corrosionMatrix = new List<IntergranularLatticeNode>();
    ...
    private void InitializeCorrosionLattice()
    {
    ...
    private void SimulateLanthanideCorrosionCycle()
    {
        ...
        for (int i = 0; i < corrosionMatrix.Count; i++)
        {
            IntergranularLatticeNode node = corrosionMatrix[i];

            // Stage 1: Moisture interaction triggers growth of the initial grey-purple oxide crust
            if (node.localizedAirExposure >= 25.0f && node.intergranularShearStrain <= 15f)
            {
                node.oxideVolumeSwell += Time.deltaTime * atmosphericHumidity * 4.5f;
            }

            // Stage 2: Continued oxygen exposure generates massive expansion mismatch strain
            if (node.oxideVolumeSwell >= 35.0f)
            {
                node.intergranularShearStrain += Time.deltaTime * (atmosphericHumidity * 5.2f);
            }
            ...
            corrosionMatrix[i] = node; // Commit updated settings back to structural index heap
        }

        // Pass normalization factors directly down to the PBR GPU shader properties
        compiledSpallationProgress = (float)spalledNodesCount / corrosionMatrix.Count;
        ...
        // Neodymium completely sheds its mechanical strength as it turns into loose, spalling powder layers.
        float normalizedStrainFactor = 1.0f - (totalInterfaceStrain / (corrosionMatrix.Count * 100f));
        dynamicStructuralIntegrity = Mathf.Lerp(pristineMetalYieldMPa * 0.01f, pristineMetalYieldMPa, normalizedStrainFactor);

        if (rb != null)
        {
            // Neodymium has an elemental mass density of 7.01 g/cm³; mass drops as crumbling layers shed off
            rb.mass = Mathf.Lerp(70.0f, 38.0f, compiledSpallationProgress);
        }

        // Particle Management Loop: Sheds fine grey-purple scales if moved under advanced decay stress
        if (compiledSpallationProgress > 0.35f && rb != null && rb.linearVelocity.magnitude > 0.5f)
        {
            if (greyPurpleOxideDustFX != null && !greyPurpleOxideDustFX.isPlaying)
            {
                greyPurpleOxideDustFX.Play();
            }
        }
    ...
    public void RegisterKineticDeformationImpact(Vector3 contactWorldPoint, float forceInputJoules)
    {
        ...
        for (int i = 0; i < corrosionMatrix.Count; i++)
        {
            ...
            if (physicalRange < 1.4f)
            {
                ...
                {
                    node.isNodeSpalled = true;
                    if (greyPurpleOxideDustFX != null) greyPurpleOxideDustFX.Emit(8);
                }
                
                corrosionMatrix[i] = node;
            }
        }
    }
    ...
    private void ExecuteCatastrophicLatticeFailure()
    {
        ...
        if (greyPurpleOxideDustFX != null)
        {
            ParticleSystem fractureCloud = Instantiate(greyPurpleOxideDustFX, transform.position, Quaternion.identity);
            var main = fractureCloud.main;
            main.startSizeMultiplier = 3.0f; // Large burst cloud of peeling, crumbly grey-purple oxide chunks
            Destroy(fractureCloud.gameObject, 4.5f);
        }

        Debug.Log($"[LANTHANIDE MATRIX FAILURE] Neodymium crystal core completely pulverized under internal oxide expansion at {compiledSpallationProgress * 100f}% decay progress.");
